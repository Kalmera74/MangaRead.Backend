...
public class MangaRepository : Repository<MangaEntity>, IMangaRepository
{
    ...
    public async Task<IEnumerable<MangaEntity>?> GetRandomAsync(int take = 1)
    {
        ...
        var totalCount = await _context.Set<MangaEntity>().CountAsync();


        var offset = new Random().Next(0, totalCount);


        var randomEntities = await _context.Set<MangaEntity>()
                            .Where(m => m.IsPublished)
                            .Skip(offset)
                            .Take(take)
                            .Include(m => m.Authors)
    ...
    {
        return await _context.Set<MangaEntity>()
                            .Where(x => x.Title.ToLower().Contains(title.ToLower()) && x.IsPublished)
                            .Include(m => m.Authors)
                            .Include(m => m.Genres)
                            .Include(m => m.Ratings)
                            .Include(m => m.CoverImage)
