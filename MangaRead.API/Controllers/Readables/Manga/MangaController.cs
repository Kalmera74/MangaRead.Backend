...
public class MangaController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<SimpleMangaDTO>>> GetNewMangas(int take)
    {
        ...
        var mangas = await _mangaRepository.GetByCreatedAtAsync(DateTime.Now.AddDays(-7), DateTime.Now, take);
        if (mangas == null || mangas.Count() == 0)
        {
            return NotFound("No Mangas Found");
        }
    ...
    public async Task<ActionResult<IEnumerable<SimpleMangaDTO>>> GetHotMangas(int take)
    {
        ...
        var mangas = await _mangaRepository.GetByUpdatedAtAsync(DateTime.Now.AddDays(-3), DateTime.Now, take);
